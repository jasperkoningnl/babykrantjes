 ...
 * Met filtering voor niet-personen (plaatsen, locaties, etc.)
 */
function parseFamousPersons(
  ...
  try {
    ...
    // Woorden die aangeven dat het GEEN persoon is (filter)
    const nonPersonIndicators = lang === 'nl'
      ? ['county', 'plaats', 'stad', 'dorp', 'gemeente', 'park', 'rivier', 'berg', 'eiland', 
         ...
         'nationaal park', 'natuurgebied']
      : ['county', 'place', 'city', 'town', 'village', 'municipality', 'park', 'river', 
         'mountain', 'island', 'area', 'region', 'province', 'state', 'district', 'township', 
         'census-designated', 'unincorporated', 'community', 'borough', 'hamlet', 'locality',
         'national park', 'mountain range', 'lake', 'creek', 'range']
    ...
    while ((match = listItemRegex.exec(sectionHtml)) !== null) {
      ...
      // Filter niet-personen op basis van beschrijving en titel
      ...
      const isNonPerson = nonPersonIndicators.some(indicator => 
        ...
        lowerLinkText.includes(indicator)
      )
      
      if (isNonPerson) {
        console.log(`[${lang}] Filtered out non-person: ${linkText}`)
        continue
      }
