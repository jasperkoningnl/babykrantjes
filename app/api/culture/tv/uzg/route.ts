...
export async function GET(request: NextRequest) {
  ...
  // - tvblik.nl: Volledig programma-overzicht met tijden (primair)
  // - kijkonderzoek.nl: Top 25 met kijkcijfers (context - fallback)
  // ============================================================================
  const daysAgo = calculateDaysAgo(dateParam)
  if (daysAgo >= 0 && daysAgo <= 7) {
    const baseUrl = request.nextUrl.origin

    // Try tvblik.nl first (volledig programma-overzicht)
    console.log(`[TV API] → Layer 2a: tvblik.nl LIVE (${daysAgo} days ago)`)
    try {
      ...
      if (response.ok) {
        ...
        // Check of er daadwerkelijk data is (geen error en programma's gevonden)
        ...
        console.log(`[TV API] tvblik.nl returned no data (error: ${data.error}), trying kijkonderzoek.nl...`)
      } else {
        console.log(`[TV API] tvblik.nl failed (HTTP ${response.status}), trying kijkonderzoek.nl...`)
      }
    } catch (error) {
      console.error('[TV API] tvblik.nl error:', error)
    }

    // Try kijkonderzoek.nl as fallback (Top 25 met kijkcijfers voor context)
    console.log(`[TV API] → Layer 2b: kijkonderzoek.nl LIVE (${daysAgo} days ago)`)
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[TV API] kijkonderzoek.nl returned no data (error: ${data.error}), falling back to Layer 3`)
      } else {
